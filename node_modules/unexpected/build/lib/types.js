...
module.exports = function (expect) {
  ...
  expect.addType({
    ...
    equal: function equal(a, b, _equal2) {
      ...
      var actualKeys = this.getKeys(a).filter(function (key) {
        return typeof _this.valueForKey(a, key) !== 'undefined';
      });
      var expectedKeys = this.getKeys(b).filter(function (key) {
        return typeof _this.valueForKey(b, key) !== 'undefined';
      });
      ...
      for (var j = 0; j < actualKeys.length; j += 1) {
        ...
        if (!_equal2(this.valueForKey(a, key), this.valueForKey(b, key))) {
          return false;
        }
      }
      return true;
    ...
    similar: function similar(a, b) {
      ...
      return aKeys.concat(bKeys).some(function (key) {
        if (_this2.hasKey(a, key) && _this2.hasKey(b, key)) {
  ...
  expect.addType({
      ...
      if (a === b) {
        ...
        if (!this.numericalPropertiesOnly) {
          var aKeys = this.getKeysNonNumerical(a).filter(function (key) {
            ...
            return typeof _this3.valueForKey(a, key) !== 'undefined';
          });
          var bKeys = this.getKeysNonNumerical(b).filter(function (key) {
            // include keys whose value is not undefined on either LHS or RHS
            return typeof _this3.valueForKey(b, key) !== 'undefined' || typeof _this3.valueForKey(a, key) !== 'undefined';
          });
    ...
    inspect: function inspect(arr, depth, output, _inspect4) {
      ...
      var inspectedItems = keys.map(function (key) {
        ...
        if (_this4.hasKey(arr, key)) {
          inspectedValue = _inspect4(_this4.valueForKey(arr, key));
        ...
        return _this4.property(output.clone(), key, inspectedValue, true);
      });
      ...
      inspectedItems.forEach(function (inspectedItem, index) {
        inspectedItem.amend(_this4.delimiter(output.clone(), index, keys.length));
      });
      if (multipleLines) {
        output.append(prefixOutput);
        if (!prefixOutput.isEmpty()) {
  ...
  expect.addType({
    ...
    inspect: function inspect(f, depth, output, _inspect7) {
      ...
      var matchSource = source.match(/^\s*((?:async )?\s*(?:\S+\s*=>|\([^)]*\)\s*=>|function\s?\w*\s*\([^)]*\)))([\s\S]*)$/);
      if (matchSource) {
