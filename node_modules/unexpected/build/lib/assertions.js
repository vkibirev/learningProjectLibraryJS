                ...
                if (type === 'moveTarget') {
                  ...
                  return output.clone().block(function () {
                    if (type === 'moveSource') {
                      ...
                      this.append(propertyOutput).append(delimiterOutput).sp().error('// should be moved');
                    } else if (type === 'insert') {
                      ...
                      this.block(function () {
                        if (type === 'remove') {
                          this.append(_propertyOutput2).append(delimiterOutput).sp().error('// should be removed');
                        } else if (type === 'equal') {
                          this.append(_propertyOutput2).append(delimiterOutput);
                        } else {
                          ...
                          if (valueDiff && valueDiff.inline) {
                            ...
                            this.append(_propertyOutput2).append(delimiterOutput).sp().annotationBlock(function () {
    ...
    return expect.promise.all([expect.promise(function () {
      if (expect.flags.exhaustively) {
        var nonOwnKeysWithDefinedValues = keys.filter(function (key) {
          return !Object.prototype.hasOwnProperty.call(subject, key) && typeof subjectType.valueForKey(subject, key) !== 'undefined';
        });
        var valueKeysWithDefinedValues = keys.filter(function (key) {
          return typeof valueType.valueForKey(value, key) !== 'undefined';
        });
        var subjectKeysWithDefinedValues = subjectKeys.filter(function (key) {
          return typeof subjectType.valueForKey(subject, key) !== 'undefined';
        });
      ...
      return expect.promise.settle(promiseByKey).then(function () {
        expect.fail({
          diff: function diff(output, _diff6, inspect, equal) {
            ...
            var subjectIsArrayLike = subjectType.is('array-like');
            var valueKeys = valueType.getKeys(value);
            var keys = subjectType.uniqueKeys(subjectKeys, valueKeys).filter(function (key) {
